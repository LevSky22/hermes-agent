...
def _format_job(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = {
        ...
        "pause_after_delivery": bool(job.get("pause_after_delivery", False)),
    }
    if job.get("script"):
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "pause_after_delivery": {
                "type": "boolean",
                "description": "For polling watchers: atomically pause the job after its first successful, non-silent external delivery. Use this instead of instructing the scheduled agent to call cronjob(action='pause'): cron sessions intentionally cannot access the cronjob tool. Silent checks, failed runs, and failed/no-target deliveries keep running. On update, False disables the policy."
            },
        },
        "required": ["action"]
    }
...
registry.register(
    ...
    handler=lambda args, **kw: (lambda _mo=_resolve_model_override(args.get("model")): cronjob(
        ...
        pause_after_delivery=args.get("pause_after_delivery"),
