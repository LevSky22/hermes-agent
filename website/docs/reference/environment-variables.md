...
| `DISCORD_THREAD_FREE_RESPONSE_CHANNELS` | Comma-separated channel IDs where mention is not required but top-level messages still auto-thread when `DISCORD_AUTO_THREAD` is enabled |
