...
| **Free-response channels** | You can make specific channels mention-free with `DISCORD_FREE_RESPONSE_CHANNELS`, or disable mentions globally with `DISCORD_REQUIRE_MENTION=false`. Messages in these channels are answered inline — auto-threading is skipped so the channel stays a lightweight chat. Use `DISCORD_THREAD_FREE_RESPONSE_CHANNELS` for mention-free ambient intake channels that should still auto-thread. |
...
:::tip
If you want a normal bot-help channel where people can talk to Hermes without tagging it every time, add that channel to `DISCORD_FREE_RESPONSE_CHANNELS`. If you want each top-level no-mention request to become its own thread, use `DISCORD_THREAD_FREE_RESPONSE_CHANNELS` instead.
:::
...
| `DISCORD_FREE_RESPONSE_CHANNELS` | No | — | Comma-separated channel IDs where the bot responds inline without requiring an `@mention`, even when `DISCORD_REQUIRE_MENTION` is `true`. |
| `DISCORD_THREAD_FREE_RESPONSE_CHANNELS` | No | — | Comma-separated channel IDs where the bot responds without requiring an `@mention`, but top-level messages still use auto-threading when `DISCORD_AUTO_THREAD` is `true`. |
...
discord:
  ...
  thread_free_response_channels: [] # Mention-free channels that still auto-thread
...
Free-response channels also **skip auto-threading** — the bot replies inline rather than spinning off a new thread per message. This keeps the channel usable as a lightweight chat surface. If you want mention-free intake with one thread per top-level request, use [`discord.thread_free_response_channels`](#discordthread_free_response_channels) instead.

#### `discord.thread_free_response_channels`

**Type:** string or list — **Default:** `[]`

Channel IDs where the bot responds to top-level messages without needing an `@mention`, while still using the normal auto-thread behavior when `discord.auto_thread` is enabled. This is useful for dedicated Hermes intake channels where most parent-channel messages are requests, but each request should be contained in its own thread.
...
Threads and DMs do not spawn nested threads. `discord.no_thread_channels` still takes precedence for channels that must always stay inline.
...
Messages sent in existing threads or DMs are unaffected by this setting. Channels listed in `discord.free_response_channels` or `discord.no_thread_channels` bypass auto-threading and get inline replies instead. Channels listed in `discord.thread_free_response_channels` stay mention-free but use auto-threading unless also listed in `discord.no_thread_channels`.
